...
export class AdminsService implements OnModuleInit {
  ...
  async getAdminSettingsSummary(): Promise<{
    ...
    // Lấy bản ghi admin_settings đầu tiên
    const adminSetting = await this.adminSettingRepository.findOne({
      where: {},
      order: { as_id: 'ASC' },
    });

    // Định nghĩa các mốc thưởng member-ref
    ...
    // Nếu chưa có, trả về giá trị mặc định với max_level = 0 và parcentage rỗng
    if (!adminSetting) {
      return {
        statusCode: 200,
        settings: {
