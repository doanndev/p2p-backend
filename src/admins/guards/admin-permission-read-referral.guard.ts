...
@Injectable()
export class AdminPermissionReadReferralGuard implements CanActivate {
  constructor(
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Check permission referral:read
    const rolePermission = await this.rolePermissionRepository
      ...
      .andWhere('p.permission_resource = :resource', {
        resource: PermissionResource.REFERRAL,
      })
    ...
    if (!rolePermission) {
      throw new ForbiddenException(
        'You do not have permission to access this resource. Required: referral read permission or Super Admin role.',
      );
    }
