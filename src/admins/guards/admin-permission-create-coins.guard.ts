...
@Injectable()
export class AdminPermissionCreateCoinsGuard implements CanActivate {
  constructor(
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const rolePermission = await this.rolePermissionRepository
      ...
      .andWhere('p.permission_resource = :resource', {
        resource: PermissionResource.COINS,
      })
    ...
    if (!rolePermission) {
      throw new ForbiddenException(
        'You do not have permission to access this resource. Required: coins create permission or Super Admin role.',
      );
    }
