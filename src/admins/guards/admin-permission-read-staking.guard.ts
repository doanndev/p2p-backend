...
@Injectable()
export class AdminPermissionReadStakingGuard implements CanActivate {
  constructor(
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const rolePermission = await this.rolePermissionRepository
      ...
      .andWhere('p.permission_resource = :resource', {
        resource: PermissionResource.STAKING,
      })
    ...
    if (!rolePermission) {
      throw new ForbiddenException(
        'You do not have permission to access this resource. Required: staking read permission or Super Admin role.',
      );
    }
