...
type NewWithdrawNotificationPayload = {
  amount: number;
  asset?: string;
  network?: string;
  txHash?: string;
...
export function getNewWithdrawNotificationEmailHtml(
  payload: NewWithdrawNotificationPayload,
): string {
  const {
    amount,
    asset = 'USDT',
              ...
              <tr>
                <td style="background: #f5f5f7 url('${backgroundImageUrl}') no-repeat center / cover; padding: 28px 22px; border-radius: 8px;">
                  <p style="margin: 0 0 14px; color: #4a4f5f; font-size: 16px; line-height: 1.55;">
                    A new withdrawal request has been recorded for your account.
                  </p>
                  <p style="margin: 0 0 18px; color: #4a4f5f; font-size: 16px; line-height: 1.55;">
                    If this request was not made by you, please contact support immediately to secure your account.
                  </p>
