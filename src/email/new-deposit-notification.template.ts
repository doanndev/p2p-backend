...
export function getNewDepositNotificationEmailHtml(
  payload: NewDepositNotificationPayload,
): string {
  const {
    amount,
    asset = 'USDT',
              ...
              <tr>
                <td style="background: #f5f5f7 url('${backgroundImageUrl}') no-repeat center / cover; padding: 28px 22px; border-radius: 8px;">
                  <p style="margin: 0 0 14px; color: #4a4f5f; font-size: 16px; line-height: 1.55;">
                    We detected a new deposit transaction in your account.
                  </p>
                  <p style="margin: 0 0 18px; color: #4a4f5f; font-size: 16px; line-height: 1.55;">
                    Please review the details below. If this activity is not yours, contact support immediately.
                  </p>
